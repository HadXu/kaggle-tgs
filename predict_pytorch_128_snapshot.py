...
for cv_num, (train_idx, val_idx) in enumerate(cv.split(data_ids, data_class)):
    ...
    for cycle in range(cycle_num):
        print('cycle:', cycle)
        net.load_state_dict(torch.load('../weights/{}_{}_lovasz_loss_clr_{}.th'.format(name, cv_num, cycle)))
        iou = do_eval(net, valid_dataset)
        print('valid iou:', iou)
        test_probs += do_test(net, test_dataset)

test_probs /= kfold * cycle_num
...
sub.to_csv('results/cv_submission_128_snapshot.csv')

np.save("results/test_probs_128_snapshot.npy", test_probs)
